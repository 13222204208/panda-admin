...
function addAsyncRoutes(arrRoutes: Array<RouteRecordRaw>) {
  ...
  arrRoutes.forEach((v: RouteRecordRaw) => {
    // 在 addAsyncRoutes 函数中，替换现有的组件映射逻辑
    if (v.meta?.frameSrc) {
      v.component = IFrame;
    } else {
      // 改进的组件映射逻辑
      ...
      // 尝试精确匹配 .vue 文件
      let matchedKey = modulesRoutesKeys.find(key => {
        // 优先匹配 index.vue
        if (key === `${componentPath}/index.vue`) return true;
        // 其次匹配 .vue 文件
        if (key === `${componentPath}.vue`) return true;
        return false;
      });

      // 如果没有精确匹配，使用原来的 includes 逻辑作为后备
      if (!matchedKey) {
        const index = modulesRoutesKeys.findIndex(ev =>
          ev.includes(componentPath)
        );
        matchedKey = modulesRoutesKeys[index];
      }

      v.component = modulesRoutes[matchedKey];
      // console.log(`路由 ${v.path} 映射到组件: ${matchedKey}`);
    }
