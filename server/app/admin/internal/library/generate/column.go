...
// 表单验证
const (
	FormValidationNone   = "none"
	FormValidationPhone  = "phone"
	FormValidationEmail  = "email"
	FormValidationNum    = "num"
	FormValidationAmount = "amount"
)

var FormValidationMap = map[string]string{
	FormValidationNone:   "无规则",
	FormValidationPhone:  "手机号",
	FormValidationEmail:  "邮箱",
	FormValidationNum:    "非零正整数",
	FormValidationAmount: "金额",
}
...
// WhereModeMap 查询条件映射
var WhereModeMap = map[string]string{
	WhereModeEq:         "等于(=)",
	WhereModeNeq:        "不等于(!=)",
	WhereModeGt:         "大于(>)",
	WhereModeGte:        "大于等于(>=)",
	...
	WhereModeLte:        "小于等于(<=)",
	WhereModeIn:         "包含(IN)",
	WhereModeNotIn:      "不包含(NOT IN)",
	WhereModeBetween:    "区间(BETWEEN)",
	WhereModeNotBetween: "不在区间(NOT BETWEEN)",
	WhereModeLike:       "模糊查询(LIKE)",
}

// WhereModeOption 查询条件选项结构体
type WhereModeOption struct {
	Label string `json:"label"`
	Value string `json:"value"`
}

// GetWhereModeOptions 返回查询条件的选项数据
func GetWhereModeOptions() []WhereModeOption {
	var options []WhereModeOption

	for _, mode := range WhereModes {
		label, exists := WhereModeMap[mode]
		if !exists {
			label = mode // 如果映射不存在，使用原值作为标签
		}

		options = append(options, WhereModeOption{
			Label: label,
			Value: mode,
		})
	}
...
// FormModeOption 表单模式选项结构体
type FormModeOption struct {
	Label string `json:"label"`
	Value string `json:"value"`
}

// GetFormModeOptions 返回表单模式的选项数据
func GetFormModeOptions() []FormModeOption {
	var options []FormModeOption

	for _, mode := range FormModes {
		label, exists := FormModeMap[mode]
		if !exists {
			label = mode // 如果映射不存在，使用原值作为标签
		}

		options = append(options, FormModeOption{
			Label: label,
			Value: mode,
		})
	}
...
// ValidationRules 表单验证规则切片
var ValidationRules = []string{
	FormValidationNone,
	FormValidationPhone,
	FormValidationEmail,
	FormValidationNum,
	FormValidationAmount,
}

// FormValidationOption 表单验证选项结构体
type FormValidationOption struct {
	Label string `json:"label"`
	Value string `json:"value"`
}

// GetFormValidationOptions 返回表单验证的选项数据
func GetFormValidationOptions() []FormValidationOption {
	var options []FormValidationOption

	for _, role := range ValidationRules {
		label, exists := FormValidationMap[role]
		if !exists {
			label = role // 如果映射不存在，使用原值作为标签
		}

		options = append(options, FormValidationOption{
			Label: label,
			Value: role,
		})
	}
